#!/usr/bin/env python3
"""Generate an editable PowerPoint slide for the AVD Private Link architecture (v2).
Corrected flow: Entra ID auth goes directly from client via public internet,
not through SD-WAN/VNet Gateway."""
...
PUBLIC_YELLOW = RGBColor(0xF9, 0xA8, 0x25)
...
def add_arrow(slide, start_x, start_y, end_x, end_y, color, width=Pt(2), dashed=False):
    conn = add_connector(slide, start_x, start_y, end_x, end_y, color, width)
    conn.line.fill.solid()
    conn.line.color.rgb = color
    ln = conn.line._get_or_add_ln()
    nsmap_a = 'http://schemas.openxmlformats.org/drawingml/2006/main'
    tail_end = etree.SubElement(ln, f'{{{nsmap_a}}}tailEnd')
...
add_text_box(slide, Inches(0.3), Inches(0.15), Inches(12), Inches(0.5),
             "Azure Virtual Desktop — Private Link Architecture (v2)",
...
sz_l = onprem_l + Inches(0.2)
sz_t = onprem_t + Inches(0.6)
sz_w, sz_h = Inches(1.8), Inches(1.3)
add_rounded_rect(slide, sz_l, sz_t, sz_w, sz_h,
                 RGBColor(0x38, 0x8E, 0x3C), DARK_GREEN,
                 "🖥️ Thin Clients\n(Shenzhen)\n🔒 10.1.1.0/24",
                 font_size=9, font_color=WHITE, bold=False, border_width=Pt(1))

# Thin Client - Dalian
dl_l = onprem_l + Inches(0.2)
dl_t = onprem_t + Inches(2.2)
dl_w, dl_h = Inches(1.8), Inches(1.3)
add_rounded_rect(slide, dl_l, dl_t, dl_w, dl_h,
                 RGBColor(0x38, 0x8E, 0x3C), DARK_GREEN,
                 "🖥️ Thin Clients\n(Dalian)\n🔒 10.2.1.0/24",
                 font_size=9, font_color=WHITE, bold=False, border_width=Pt(1))

# SD-WAN circle
sdwan_cx, sdwan_cy = Inches(3.4), Inches(3.5)
sdwan_r = Inches(0.7)
...
# Azure Entra ID (top-left, between OnPrem and CE2)
entra_l = Inches(1.5)
entra_t = Inches(0.1)
...
# Key anchor points
sz_r = onprem_l + onprem_w
dl_r = onprem_l + onprem_w
sz_cy = sz_t + sz_h / 2
dl_cy = dl_t + dl_h / 2
sz_top = sz_t
dl_top = dl_t
...
entra_bot = entra_t + entra_h

# ── ① Client → Entra ID (Public Internet, dashed yellow) ──
add_arrow(slide, sz_r - Inches(0.5), sz_t, entra_l + Inches(0.3), entra_bot,
          PUBLIC_YELLOW, Pt(3), dashed=True)
add_arrow(slide, dl_r - Inches(0.5), dl_t, entra_l + Inches(0.5), entra_bot,
          PUBLIC_YELLOW, Pt(3), dashed=True)
add_step_label(slide, Inches(0.5), Inches(0.82), "①",
               "Entra ID Auth\n⚠️ Public Internet", PUBLIC_YELLOW)

# ── ② Entra ID → Client (Token returned, dashed yellow) ──
add_arrow(slide, entra_l + Inches(0.1), entra_bot,
          sz_l + sz_w / 2, sz_t,
          PUBLIC_YELLOW, Pt(2), dashed=True)
add_arrow(slide, entra_l + Inches(0.3), entra_bot,
          dl_l + dl_w / 2, dl_t,
          PUBLIC_YELLOW, Pt(2), dashed=True)
add_step_label(slide, Inches(2.7), Inches(0.82), "②",
               "Token Returned\n⚠️ Public Internet", PUBLIC_YELLOW)

# ── ③ Client → SD-WAN → VNet Gateway (with token, private) ──
...
add_step_label(slide, Inches(2.55), sz_cy - Inches(0.35), "③",
               "With Token · SD-WAN", STEP_BLUE)

# ③ SD-WAN → VNet Gateway
add_arrow(slide, sdwan_r_edge, sdwan_cy, gw_l, gw_cy_mid, SDWAN_GREEN, Pt(3))
add_step_label(slide, Inches(4.1), Inches(2.5), "③",
               "SD-WAN Private Transit", STEP_BLUE)

# ── ④ VNet Gateway → PE Workspace (Feed Discovery) ──
add_arrow(slide, gw_cx, gw_t + gw_h, pe_ws_cx, pe_ws_t, STEP_BLUE)
add_step_label(slide, Inches(5.3), Inches(1.75), "④",
               "Feed Discovery", STEP_BLUE)

# ── ⑤ PE Workspace → PE Host Pool ──
add_arrow(slide, pe_ws_l + pe_ws_w, pe_ws_cy, pe_hp_l, pe_hp_cy, STEP_BLUE)
add_step_label(slide, Inches(6.6), Inches(2.8), "⑤",
               "RDP Session Request", STEP_BLUE)
...
add_step_label(slide, Inches(8.7), Inches(2.0), "⑥",
               "RDP Reverse Connect", STEP_BLUE)

# ── ⑦ Session Host → App Server ──
add_arrow(slide, sh_vnet_l, sh_cy_bot, biz_l + biz_w, app_cy - Inches(0.15), STEP_BLUE)
add_step_label(slide, Inches(8.3), Inches(4.0), "⑦",
               "VNet Peering to App", STEP_BLUE)

# ── ⑧ App Server → Session Host ──
add_arrow(slide, biz_l + biz_w, app_cy + Inches(0.15), sh_vnet_l, sh_cy_bot + Inches(0.3), STEP_BLUE)
add_step_label(slide, Inches(8.3), Inches(4.55), "⑧",
               "Return Data", STEP_BLUE)

# ── ⑨ RDP Shortpath (Session Host → GW → SD-WAN → Client) ──
add_arrow(slide, sh_vnet_l, sh_cy_top + Inches(0.2),
          gw_l + gw_w, gw_cy_mid, SHORTPATH_ORANGE, Pt(3))
add_step_label(slide, Inches(8.3), Inches(1.15), "⑨",
               "RDP Shortpath · UDP", STEP_RED)

add_arrow(slide, gw_l, gw_cy_mid, sdwan_r_edge, sdwan_cy - Inches(0.3), SHORTPATH_ORANGE, Pt(3))
add_step_label(slide, Inches(3.9), Inches(1.2), "⑨",
               "SD-WAN Transit", STEP_RED)
...
# Dashed yellow = public auth
add_connector(slide, legend_l + Inches(0.05), legend_t + Inches(0.4),
              legend_l + Inches(0.6), legend_t + Inches(0.4), PUBLIC_YELLOW, Pt(3))
add_text_box(slide, legend_l + Inches(0.65), legend_t + Inches(0.28),
             Inches(2.5), Inches(0.25),
             "①② Entra ID Auth (⚠️ Public Internet)", font_size=8, font_color=BLACK)

# Blue arrow = private flow
add_connector(slide, legend_l + Inches(0.05), legend_t + Inches(0.7),
              legend_l + Inches(0.6), legend_t + Inches(0.7), STEP_BLUE, Pt(2))
add_text_box(slide, legend_l + Inches(0.65), legend_t + Inches(0.58),
             Inches(2.5), Inches(0.25),
             "③~⑧ Private Connection Flow", font_size=8, font_color=BLACK)

# Orange arrow = shortpath
add_connector(slide, legend_l + Inches(0.05), legend_t + Inches(1.0),
              legend_l + Inches(0.6), legend_t + Inches(1.0), SHORTPATH_ORANGE, Pt(3))
add_text_box(slide, legend_l + Inches(0.65), legend_t + Inches(0.88),
             Inches(2.5), Inches(0.25),
             "⑨ RDP Shortpath (Direct UDP · Private)", font_size=8, font_color=BLACK)

# Green arrow = SD-WAN
add_connector(slide, legend_l + Inches(0.05), legend_t + Inches(1.3),
              legend_l + Inches(0.6), legend_t + Inches(1.3), SDWAN_GREEN, Pt(3))
...
# Save
# ══════════════════════════════════════════════════════════════════
output_path = "/Users/tema/projects/avd-privatelink/AVD-PrivateLink-Architecture-v2.pptx"
prs.save(output_path)
print(f"✅ PPT v2 saved to: {output_path}")
